...
以下决策已确认，可直接进入 MVP 实现与任务拆分：

| 议题 | 决策 |
|------|------|
| 平台库选用机制 | 平台库资产不进入 Asset Library；在合成页直接选用作为参考图；Generation Record 保留当时引用快照。 |
| 资产删除 | 资产库采用软删除；OSS 文件不删除；历史 Generation Record 仍可查看当时参考图与相关元数据。 |
| 同一 Output 重复入库 | 同一 Generation Output 在未被移除前仅允许入库一次；若已从 Try-on Image 分库移除，可再次入库。 |
| 生成张数上限 | 单次 Try-on Job 生成张数范围为 1～4，默认 1。 |
| Prompt Template 具体内容 | MVP 采用代码内置单模板（v1）；根据所选素材自动填充并允许 Brand Operator 在发起前手动编辑；Generation Record 保留模板版本与最终实际提示词。 |
